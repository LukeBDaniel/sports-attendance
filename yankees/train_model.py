...
train_model.py
--------------
Loads data/yankees_games.csv, trains a Gradient Boosting attendance model,
evaluates it, and saves model.pkl for use in the Streamlit app.

Train/test split: random 80/20 across all available seasons (2000–2025,
...
FEATURES = [
    "season",               # captures long-term attendance trends
    "day_of_week",
    "month",
    "is_weekend",
    "is_evening",
...
def main():
    ...
    ax.scatter(y_test, gb_preds, alpha=0.5, s=30, color="#132448")
    lims = [min(y_test.min(), gb_preds.min()) * 0.9,
            max(y_test.max(), gb_preds.max()) * 1.05]
    ax.plot(lims, lims, "--", color="#C4011D", lw=1.5, label="Perfect prediction")
    ...
    ax.set_title(f"Gradient Boosting: Actual vs Predicted Fill Rate\n"
                 f"MAE = {gb_metrics['mae']:.3f} | R² = {gb_metrics['r2']:.3f}")
    ax.legend()
    plt.tight_layout()
    ...
    print(f"Gradient Boosting MAE {gb_metrics['mae']:.4f} fill rate "
          f"vs Ridge MAE {baseline_metrics['mae']:.4f}")
